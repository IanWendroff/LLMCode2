...
static void release_locked_keys(const std::vector<int>& keys)
{
    for (int key : keys) {
        release_write(key);
    }
...
bool put_many(std::vector<std::pair<int, std::string>> vec){
    ...
    if (participants.count(self_id)){ //if self_id is in the participants set (coordinator node is a participant)
        ...
        if (all_local_locked){
            ...
            if (!txn.locked_keys.empty()){
                ...
                release_locked_keys(txn.locked_keys);
            }
            all_commit = false;
        }
    }
    ...
    if (participants.count(self_id)) {
        ...
        if (all_commit && all_local_locked && has_txn) {
...
void handle_client(int sock)
{
    ...
    else if (line.rfind("GET", 0) == 0)
 {
        ...
        if (txn_id == static_cast<uint64_t>(-1)) {
            send_line(sock, "VOTE_ABORT -1\n");
            continue;
        }
